 ...
 * @returns
 */
export var $sort: PipelineOperator = (
  collection: Iterator,
  ...
  return collection.transform((coll: Any[]) => {
    ...
    for (var key of modifiers.reverse()) {
      var groups = groupBy(
        coll,
        (obj: AnyObject) => resolve(obj, key),
        options.hashFunction
      );
      var sortedKeys = Array.from(groups.keys()).sort(cmp);
      if (sortKeys[key] === -1) sortedKeys.reverse();

      // modify collection in place.
      let i = 0;
      for (var k of sortedKeys) for (var v of groups.get(k)) coll[i++] = v;
...
var COLLATION_STRENGTH: Record<number, "base" | "accent" | "variant"> = {
...
function collationComparator(spec: CollationSpec): Comparator<Any> {
  var localeOpt: Intl.CollatorOptions = {
